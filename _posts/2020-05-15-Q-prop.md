...
&= \mathbb{E}_{\rho_{\pi}}[\mathbb{E}_{\pi}[\hat{A}(s_t, a_t)^{2}] 
- 2\eta(s_t)\mathbb{E}_{\pi}[\hat{A}(s_t, a_t)\bar{A}(s_t, a_t)]
+ \eta(s_t)^{2}\mathbb{E}_{\pi}[\bar{A}(s_t, a_t)^{2}]] \\
&= Var + \mathbb{E}_{\rho_{\pi}}[- 2\eta(s_t) Cov_{a_t}(\hat{A}(s_t, a_t), \bar{A}(s_t, a_t)) + \eta(s_t)^{2} Var_{a_t}(\bar{A}(s_t, a_t))]
\end{align}
$$

$\bar{A}(s_t,a_t) = \nabla_a Q_w (s_t, a)|_{a = \mu_{\theta}(s_t)}(a_t - \mu_{\theta}(s_t))$ 이므로  $\sum_{\theta}(s_t)$를 stochastic policy $\pi_{\theta}$의 covariance matrix라고 할 때 analytic gradient term의 variance 추가항은 아래와 같다.
$$
\begin{align}
