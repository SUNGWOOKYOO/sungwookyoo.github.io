...
$\mathbf{W}_{FC}^1 \in \mathbb{R}^{500 \times (n_{l-att} + n_w \times n_{g-att})}$, <br>
$\mathbf{W}_{FC}^2 \in \mathbb{R}^{50 \times 500}$ )<br>

($\oplus$는 concatenation 연산,  <br>
$ \mathbf{z}_{out}^1 \in \mathbb{R}^{n_{l-att} + n_w \times n_{g-att}}$, <br>
$g(.) = ReLu(.)$) <br>
$$
