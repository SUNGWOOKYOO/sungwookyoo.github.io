...
$  \mathcal{V}  $ : a set of words, $|\mathcal{V}|$ : the size of the vocabulary   
...
(FC layer에서 layer size = $[500,50]$으로 가정하면, $\mathbf{W}_{FC}^1 \in \mathbb{R}^{500 \times (n_{l-att} + n_w \times n_{g-att})}$, $\mathbf{W}_{FC}^2 \in \mathbb{R}^{50 \times 500}$ )
